...
unsigned long lastFrequencyUpdateTime = 0;
...
float estimateSignalFrequency() {
  float currentValue = readvADCValue();
  unsigned long currentTime = millis();
  static unsigned long lastZeroCrossingTime = 0;
  static bool lastPositive = false;

  bool currentPositive = currentValue > 1.65; // Assumindo que 1.65V é o nível médio
  if (currentPositive && !lastPositive) {
    unsigned long period = currentTime - lastZeroCrossingTime;
    lastZeroCrossingTime = currentTime;
    lastPositive = currentPositive;
    if (period > 0) {
      return 1000.0 / period; // Frequência em Hz
    }
  }
  lastPositive = currentPositive;
  return 0;
...
void adjustSamplingRate() {
  if (millis() - lastFrequencyUpdateTime > 1000) { // Atualizar a cada 1 segundo
    lastFrequencyUpdateTime = millis();
    float frequency = estimateSignalFrequency();
    if (frequency > 0) {
      acquisitionRate = max(static_cast<float>(2 * (1000 / frequency)), 1.0f); // Taxa de amostragem deve ser pelo menos 2x a frequência
    }
  }
}
